...
const growthPoints = [
  {
    number: "01",
    problem: "В старом каталоге товары сгруппированы хаотично — чтобы найти нужную позицию, приходилось пролистывать всё подряд.",
    change: "Выстраиваем четкую навигацию «Категория → Подкатегория → Товар» и добавляем наглядный путь возврата назад.",
    result: "Закупщик сразу понимает, в каком разделе находится, и за секунды переходит к нужной позиции.",
  },
  {
    number: "02",
    problem: "Фильтры были спрятаны глубоко в разделах — подобрать товар по параметрам (IP, размер, материал) было неудобно.",
    change: "Сделали отдельный фильтр: задаёте требования к изделию и сразу получаете готовый список.",
    result: "Снабженец легко подбирает аналог или нужный бокс, даже если не помнит точный артикул.",
  },
  {
    number: "03",
    problem: "В старых карточках характеристики разнесены по вкладкам, и сложно быстро выгрузить товар в заявку.",
    change: "Вывели все технические данные на главный экран и добавили быструю кнопку «В спецификацию».",
    result: "Закупщик сразу формирует список нужных товаров с объемами, а не просто читает описание.",
  },
  {
    number: "04",
    problem: "Раньше при оформлении заявки приходилось вручную переписывать артикулы в форму обратной связи.",
    change: "Связали спецификацию, количество позиций и форму расчета в один клик.",
    result: "Менеджер сразу получает полный список товаров с количествами и рассчитывает цену без лишних звонков.",
  },
];
    ...
    <header className="site-header">
      <div className="header-inner">
        <Link href="/" className="brand-lockup" onClick={() => setMobileOpen(false)}>
          ...
          <span className="brand-caption">ПРОТОТИП ЦИФРОВОГО КАТАЛОГА</span>
        </Link>
...
function Footer() {
  return <footer className="site-footer">
    <div className="footer-grid wrap">
      ...
      <div><div className="footer-label">Навигация</div><Link href="/catalog">Каталог</Link><Link href="/selector">Подбор продукции</Link><Link href="/partner">Личный кабинет партнёра</Link></div>
      ...
      <div className="footer-cta"><span className="mono">ЦИФРОВОЙ КАТАЛОГ B2B / 2024</span><Link className="text-link" href="/specification">Собрать спецификацию <ArrowUpRight size={15} /></Link></div>
    </div>
  ...
  return <main className="page-enter">
    <section className="hero wrap">
      <div className="hero-copy">
        <div className="eyebrow"><span className="eyebrow-dot" /> UPLAST / ПРОТОТИП КАТАЛОГА</div>
        <h1>Продукция,<br /><em>с которой</em><br />легче работать.</h1>
        <p className="hero-lead">Каталог электромонтажной продукции «УралОмегаПласт»: от точного подбора по характеристикам до отправки оптовой заявки в пару кликов.</p>
        <div className="hero-actions"><Link className="button button-primary" href="/catalog">Открыть каталог <ArrowRight size={17} /></Link><Link className="button button-ghost" href="/redesign">Оценить концепцию <ArrowDownRight size={17} /></Link></div>
        <div className="hero-meta"><span><b>01</b> Понятная навигация</span><span><b>02</b> Подбор по параметрам</span><span><b>03</b> Спецификация к заявке</span></div>
      </div>
      <div className="hero-visual">
        <div className="hero-grid-label mono">UPLAST CATALOG / 2024</div>
        <div className="hero-orbit orbit-one" /><div className="hero-orbit orbit-two" />
        <div className="hero-object"><img src={STORAGE_HERO} alt="Электромонтажная продукция Uplast" /><div className="object-caption"><span>UPLAST / УРАЛОМЕГАПЛАСТ</span><span>Системы электромонтажа</span></div></div>
    ...
    <section className="signal-strip"><div className="wrap signal-inner"><span className="signal-title">СОХРАНЯЕМ НАДЁЖНОСТЬ — <strong>УПРОЩАЕМ ЗАКАЗ</strong></span><span className="signal-note">Привычный ассортимент заводского качества, быстрый подбор и прямой расчёт <ArrowRight size={15} /></span></div></section>

    <section className="intro-section wrap"><div className="section-kicker">01 / НАША ЦЕЛЬ</div><div className="intro-grid"><h2>Uplast уже знает<br /><em>свой продукт.</em></h2><div><p className="intro-lead">Новый сайт помогает инженеру и снабженцу быстро подобрать продукцию под ТЗ и за пару секунд отправить список на расчёт оптовой цены.</p><div className="intro-facts"><div><img src={STORAGE_PIC1} alt="" /><span>Производим электромонтажную продукцию: коробки, боксы, щиты, трубы и металлорукав.</span></div><div><img src={STORAGE_PIC3} alt="" /><span>Разрабатываем и модернизируем продукцию, выполняем заказы по индивидуальным чертежам.</span></div><div><img src={STORAGE_PIC4} alt="" /><span>Доставляем продукцию по России и в страны СНГ.</span></div></div></div></div></section>

    <section className="category-section wrap"><div className="section-heading-row"><div><div className="section-kicker">02 / АССОРТИМЕНТ</div><h2>Система продукции</h2></div><Link className="text-link" href="/catalog">Весь каталог <ArrowUpRight size={16} /></Link></div><div className="category-grid">{categories.slice(0, 6).map((category, index) => <CategoryCard key={category.name} category={category} index={index} />)}</div></section>

    <section className="selector-teaser"><div className="wrap selector-teaser-grid"><div><div className="section-kicker light">03 / ПОДБОР ПО ПАРАМЕТРАМ</div><h2>Ищите не по памяти.<br /><em>Ищите по задаче.</em></h2><p>Выберите категорию, степень защиты, материал или размеры — и соберите короткий список подходящих позиций.</p><Link className="button button-light" href="/selector">Открыть подбор <ArrowRight size={17} /></Link></div><SelectorMini addToSpec={addToSpec} /></div></section>

    <section className="workflow-section wrap"><div className="section-heading-row"><div><div className="section-kicker">04 / КАК ЭТО РАБОТАЕТ</div><h2>От позиции к запросу</h2></div><span className="heading-note">Прямой путь от выбора товара до готовой заявки <ArrowRight size={15} /></span></div><div className="workflow-line"><WorkflowStep number="01" icon={<Search size={18} />} title="Найти" text="Каталог и фильтр по параметрам" /><WorkflowStep number="02" icon={<Package size={18} />} title="Сравнить" text="Все характеристики перед глазами" /><WorkflowStep number="03" icon={<ClipboardList size={18} />} title="Собрать" text="Спецификация с точным количеством" /><WorkflowStep number="04" icon={<FileText size={18} />} title="Запросить" text="Расчет оптовой стоимости" /></div><div className="workflow-cta"><div><span className="mono">ВЕДОМОСТЬ МАТЕРИАЛОВ</span><h3>Соберите нужные позиции<br />и отправьте список отделу продаж на расчёт.</h3></div><Link className="button button-primary" href="/specification">Перейти к спецификации <ArrowRight size={17} /></Link></div></section>

    <section className="growth-section"><div className="wrap"><div className="section-heading-row"><div><div className="section-kicker light">05 / ЧТО МЫ УЛУЧШИЛИ</div><h2>Точки роста,<br /><em>которые можно показать.</em></h2></div><Link className="text-link light-link" href="/redesign">Вся концепция <ArrowUpRight size={16} /></Link></div><div className="growth-grid">{growthPoints.slice(0, 3).map((point) => <GrowthCard key={point.number} point={point} />)}</div></div></section>

    <section className="partner-teaser wrap"><div className="partner-teaser-card"><div className="partner-copy"><div className="section-kicker">06 / ЛИЧНЫЙ КАБИНЕТ ПАРТНЁРА</div><h2>Кабинет партнёра<br /><em>как следующий шаг.</em></h2><p>Макет личного кабинета дилера: здесь удобно отслеживать статусы заявок, скачивать сертификаты и хранить спецификации.</p><Link className="button button-primary" href="/partner">Открыть макет <ArrowUpRight size={17} /></Link></div><div className="partner-preview"><div className="preview-top"><span className="mono">UPLAST / ЛИЧНЫЙ КАБИНЕТ</span><span className="preview-status"><i /> ПРОТОТИП</span></div><div className="preview-layout"><div className="preview-sidebar"><span className="selected"><Grid2X2 size={14} /> Обзор</span><span><ClipboardList size={14} /> Мои запросы</span><span><FileText size={14} /> Документы</span></div><div className="preview-main"><div className="preview-greeting">Добрый день, партнёр <span>→</span></div><div className="preview-cards"><div><small>ЗАПРОСЫ</small><b>—</b></div><div><small>СПЕЦИФИКАЦИИ</small><b>—</b></div></div><div className="preview-chart"><span>ПРОСТРАНСТВО ДЛЯ СОВМЕСТНОЙ РАБОТЫ</span><div className="chart-line" /></div></div></div></div></div></section>

    <section className="contact-strip"><div className="wrap contact-strip-inner"><div><div className="section-kicker">ОСТАЛИСЬ ВОПРОСЫ?</div><h2>Нужно подобрать<br /><em>решение?</em></h2></div><div className="contact-strip-actions"><p>ООО «УралОмегаПласт»<br />info@u-plast.ru · +7 (34249) 3-63-00</p><Link className="button button-primary" href="/contacts">Контакты и реквизиты <ArrowRight size={17} /></Link></div></div></section>
  </main>;
}
...
function ProductPage({ addToSpec }: { addToSpec: (product: Product) => void }) {
  ...
  return <main className="subpage page-enter"><div className="wrap breadcrumb"><Link href="/">Главная</Link><ChevronRight size={14} /><Link href="/catalog">Каталог</Link><ChevronRight size={14} /><span>{product.category}</span></div><section className="product-detail wrap"><div className="detail-image"><img src={product.image || STORAGE_CATALOG} alt={product.name} /><div className="detail-grid-mark mono">PRODUCT / {product.article}</div></div><div className="detail-copy"><span className="eyebrow"><span className="eyebrow-dot" /> КАРТОЧКА ТОВАРА</span><div className="detail-category">{product.category} <ArrowRight size={14} /> {product.subcategory}</div><h1>{product.name}</h1><p className="detail-description">{product.description}</p><div className="detail-table"><DetailRow label="Артикул" value={product.article} /><DetailRow label="Единица измерения" value={product.unit} />{product.ip && <DetailRow label="Степень защиты" value={product.ip} />} {product.size && <DetailRow label="Размеры" value={product.size} />} {product.material && <DetailRow label="Материал" value={product.material} />} {product.color && <DetailRow label="Цвет корпуса" value={product.color} />}</div><div className="detail-actions"><button className="button button-primary" onClick={() => addToSpec(product)}>Добавить в спецификацию <Plus size={17} /></button><Link className="button button-ghost" href="/specification">Открыть спецификацию <ArrowUpRight size={17} /></Link></div><div className="detail-note"><Check size={16} /> Добавление в спецификацию не обязывает к покупке — это формирование предварительной заявки.</div></div></section><section className="detail-bottom wrap"><div><span className="section-kicker">УДОБСТВО ДЛЯ ЗАКУПКИ</span><h2>Все характеристики<br /><em>перед глазами.</em></h2></div><div className="detail-context"><p>Вся техническая информация, размеры и степень защиты выведены на главный экран. Можно сразу выбрать нужный объем и выгрузить позицию в общее КП.</p><Link className="text-link" href="/selector">Попробовать подбор <ArrowRight size={15} /></Link></div></section></main>;
}
...
function SpecificationPage({ spec, updateQty, removeFromSpec, clearSpec }: { spec: SpecItem[]; updateQty: (id: string, delta: number) => void; removeFromSpec: (id: string) => void; clearSpec: () => void }) {
  ...
  return <main className="subpage page-enter"><PageIntro kicker="ВЕДОМОСТЬ МАТЕРИАЛОВ" title={<>Спецификация<br /><em>к запросу.</em></>} text="Ваш список выбранных товаров. Проверьте количества и отправьте заявку на расчёт оптовой стоимости и сроков доставки." action={<Link className="button button-ghost" href="/catalog">Добавить ещё <Plus size={17} /></Link>} /><section className="spec-layout wrap"><div className="spec-list-card"><div className="spec-card-head"><div><span className="mono">ВАША СПЕЦИФИКАЦИЯ</span><h2>{spec.length} <small>позиций</small></h2></div>{spec.length > 0 && <button className="text-button danger" onClick={clearSpec}>Очистить список</button>}</div>{spec.length === 0 ? <div className="spec-empty"><div className="empty-icon"><ClipboardList size={24} /></div><h3>Спецификация пока пуста</h3><p>Добавьте продукцию из каталога или технического подбора, чтобы показать менеджеру состав запроса.</p><Link className="button button-primary" href="/catalog">Открыть каталог <ArrowRight size={17} /></Link></div> : <div className="spec-items">{spec.map((item) => <div className="spec-item" key={item.id}><div className="spec-item-image"><img src={item.image || STORAGE_CATALOG} alt="" /></div><div className="spec-item-copy"><span className="mono">{item.article} · {item.unit}</span><h3>{item.name}</h3><div className="product-chips">{item.ip && <span>{item.ip}</span>}{item.size && <span>{item.size}</span>}{item.material && <span>{item.material}</span>}</div></div><div className="qty-control"><button onClick={() => updateQty(item.id, -1)} aria-label="Уменьшить"><Minus size={14} /></button><b>{item.qty}</b><button onClick={() => updateQty(item.id, 1)} aria-label="Увеличить"><Plus size={14} /></button></div><button className="remove-item" onClick={() => removeFromSpec(item.id)} aria-label="Удалить позицию"><X size={17} /></button></div>)}</div>}</div><div className="quote-card"><div className="quote-card-top"><span className="mono">РАСЧЁТ ОПТОВОГО ЗАКАЗА</span><FileText size={19} /></div><h2>Запросить<br /><em>оптовый расчёт.</em></h2><p>Укажите контактные данные — менеджер получит точный список товаров и сразу вернется с расчетом и сроками.</p><form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); toast.success("Запрос подготовлен", { description: "Это демонстрационный сценарий — данные никуда не отправляются." }); }}><label>Имя и фамилия<input required placeholder="Как к вам обращаться" /></label><label>Компания<input required placeholder="Название компании" /></label><label>Рабочий e-mail<input type="email" required placeholder="name@company.ru" /></label><label>Комментарий<textarea placeholder="Дополнительные требования или вопрос" rows={3} /></label><button className="button button-light full-width" disabled={spec.length === 0}>{submitted ? <><Check size={16} /> Запрос подготовлен</> : <>Отправить запрос <ArrowRight size={17} /></>}</button></form><div className="quote-privacy"><ShieldCheck size={14} /> Демонстрационная форма без реальной отправки</div></div></section></main>;
}

function PartnerPage() {
  const [tab, setTab] = useState("Обзор");
  const tabs = ["Обзор", "Мои запросы", "Мои спецификации", "Документы", "Профиль компании"];
  return <main className="subpage page-enter"><PageIntro kicker="ЛИЧНЫЙ КАБИНЕТ ПАРТНЁРА" title={<>Кабинет<br /><em>партнёра.</em></>} text="Макет личного кабинета дилера. Раздел показан как прототип будущей системы для удобной работы B2B-партнёров." action={<Link className="button button-primary" href="/specification">Перейти к спецификации <ArrowRight size={17} /></Link>} /><section className="partner-dashboard wrap"><aside className="dashboard-sidebar"><div className="dashboard-brand"><img src={STORAGE_LOGO} alt="Uplast" /><span className="partner-badge">ПРОТОТИП</span></div><div className="dashboard-company"><div className="company-avatar">К</div><div><b>Компания партнёра</b><span>Рабочее пространство</span></div></div><div className="dashboard-nav">{tabs.map((item, index) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{index === 0 ? <Grid2X2 size={15} /> : index === 1 ? <ClipboardList size={15} /> : index === 2 ? <FileText size={15} /> : index === 3 ? <ShieldCheck size={15} /> : <UserRound size={15} />}{item}{tab === item && <ChevronRight size={14} />}</button>)}</div><div className="dashboard-sidebar-foot"><span className="mono">UPLAST / B2B</span><span>ПРОТОТИП</span></div></aside><div className="dashboard-main"><div className="dashboard-top"><div><span className="mono">ЛИЧНЫЙ КАБИНЕТ · {tab.toUpperCase()}</span><h2>{tab === "Обзор" ? <>Добрый день,<br /><em>партнёр.</em></> : tab}</h2></div><div className="dashboard-top-actions"><span className="dashboard-live"><i /> ПРОТОТИП</span><button className="avatar-button">К</button></div></div>{tab === "Обзор" ? <PartnerOverview /> : <PartnerPlaceholder tab={tab} />}</div></section></main>;
}

function PartnerOverview() { return <><div className="dashboard-stat-grid"><div><span>МОИ ЗАПРОСЫ</span><b>—</b><small>Будут отображаться здесь</small></div><div><span>СПЕЦИФИКАЦИИ</span><b>—</b><small>Состав и количество позиций</small></div><div><span>ДОКУМЕНТЫ</span><b>—</b><small>Сертификаты и материалы</small></div></div><div className="dashboard-content-grid"><div className="dashboard-panel"><div className="panel-heading"><h3>Последние действия</h3><span className="mono">ИСТОРИЯ</span></div><div className="dashboard-empty"><Sparkles size={18} /><p>Здесь появится история работы<br /><span>с запросами и спецификациями</span></p></div></div><div className="dashboard-panel accent-panel"><div className="panel-heading"><h3>Быстрый старт</h3><span className="mono">ДАЛЕЕ</span></div><p>Соберите спецификацию из каталога, чтобы передать менеджеру понятный состав запроса.</p><Link className="text-link" href="/catalog">Открыть каталог <ArrowUpRight size={15} /></Link></div></div></>; }
...
function RedesignPage() { return <main className="subpage page-enter"><PageIntro kicker="КОНЦЕПЦИЯ ОБНОВЛЕНИЯ" title={<>Не просто новый вид.<br /><em>Новый способ работать.</em></>} text="Четыре ключевых улучшения навигации и каталога, которые сделают работу с сайтом удобной и быстрой." action={<Link className="button button-primary" href="/catalog">Попробовать каталог в деле <ArrowRight size={17} /></Link>} /><section className="growth-detail-section wrap"><div className="section-heading-row"><div><div className="section-kicker">ЧТО И КАК МЫ УЛУЧШИЛИ</div><h2>Точки роста</h2></div><span className="heading-note">Ключевые преимущества нового сайта <ArrowRight size={15} /></span></div><div className="growth-detail-grid">{growthPoints.map((point) => <GrowthCard key={point.number} point={point} />)}</div></section><section className="interface-section"><div className="wrap"><div className="section-heading-row"><div><div className="section-kicker light">УДОБСТВО / 01</div><h2>Новая структура<br /><em>каталога.</em></h2></div><span className="heading-note light-note">Было → Стало</span></div><div className="flow-compare"><div className="flow-old"><span className="mono">БЫЛО (СТАРЫЙ САЙТ)</span><div className="old-screen"><div className="old-header"><span>УралОмегаПласт</span><span>Продукция</span></div><div className="old-body"><div className="old-sidebar"><span>Продукция</span><span>Услуги</span><span>Вторсырьё</span></div><div className="old-list"><b>Наша продукция</b><span>Боксы пластиковые</span><span>Коробка электромонтажная</span><span>Труба гладкая</span><span>...</span></div></div></div><div className="flow-caption"><span>Список категорий</span><p>Пользователь видит ассортимент, но следующий шаг не всегда очевиден.</p></div></div><div className="flow-arrow"><ArrowRight size={25} /></div><div className="flow-new"><span className="mono">СТАЛО (НОВЫЙ КАТАЛОГ)</span><div className="new-screen"><div className="new-header"><span>КАТАЛОГ / БОКСЫ</span><span>IP30 · 6 позиций</span></div><div className="new-body"><div className="new-crumb">Главная / Каталог / Боксы пластиковые / Наружная установка</div><div className="new-main"><div className="new-filters"><span>ФИЛЬТРЫ</span><i>Степень защиты</i><i>Материал</i><i>Размеры</i></div><div className="new-results"><b>Подходящие позиции</b><div className="new-product-line"><img src={STORAGE_CATALOG} alt="" /><span>Бокс для автоматических выключателей</span><button><Plus size={12} /></button></div><div className="new-product-line"><img src={STORAGE_POPULAR} alt="" /><span>Бокс для наружной установки</span><button><Plus size={12} /></button></div></div></div></div></div><div className="flow-caption"><span>Каталог как рабочий инструмент</span><p>Цепочка навигации, фильтры, технические данные и кнопка заявки собраны на одном экране.</p></div></div></div></div></section><section className="interface-section light-interface"><div className="wrap"><div className="section-heading-row"><div><div className="section-kicker">УДОБСТВО / 02</div><h2>Карточка, которая<br /><em>продолжает путь.</em></h2></div><span className="heading-note">Карточка товара → Спецификация</span></div><div className="detail-compare"><div className="detail-before"><span className="mono">КАРТОЧКА / БЫЛО</span><div className="before-card"><div className="before-image"><img src={STORAGE_HERO} alt="" /></div><div><span className="mono">030-070</span><h3>Коробка распаячная двухкомпонентная для прямого монтажа 80х80х40</h3><p>Описание</p><p>Характеристики</p></div></div></div><div className="compare-arrow"><ArrowRight size={25} /></div><div className="detail-after"><span className="mono">КАРТОЧКА / СТАЛО</span><div className="after-card"><div className="after-image"><img src={STORAGE_HERO} alt="" /></div><div><span className="mono">030-070 · IP66</span><h3>Коробка распаячная двухкомпонентная для прямого монтажа 80х80х40</h3><div className="after-specs"><span>HF-безгалогенный</span><span>Пыль и влага</span><span>Прямой монтаж</span></div><button>Добавить в спецификацию <Plus size={14} /></button></div></div></div></div><div className="callout-row"><div><Check size={17} /><span>Характеристики в зоне решения</span></div><div><Check size={17} /><span>Кнопка добавления в заявку</span></div><div><Check size={17} /><span>Контекст не теряется</span></div></div></div></section></main>; }

function ContactsPage() { return <main className="subpage page-enter"><PageIntro kicker="КОНТАКТЫ / КОМПАНИЯ" title={<>Давайте обсудим<br /><em>следующий шаг.</em></>} text="Контактная информация ООО «УралОмегаПласт» из текущего сайта — без изменения формулировок и реквизитов." action={<a className="button button-primary" href="mailto:info@u-plast.ru">Написать на e-mail <ArrowUpRight size={17} /></a>} /><section className="contacts-grid wrap"><div className="contact-card"><span className="mono">ООО «УРАЛОМЕГАПЛАСТ»</span><h2>Производственная<br /><em>база и офис.</em></h2><div className="contact-list"><div><span>Адрес</span><p>618900, Пермский край,<br />г. Лысьва, ул. Революции, 5</p></div><div><span>Телефоны</span><p><a href="tel:+73424936300">+7 (34249) 3-63-00</a><br />3-63-01, 3-63-02</p></div><div><span>E-mail</span><p><a href="mailto:info@u-plast.ru">info@u-plast.ru</a></p></div></div></div><div className="contact-card dark-contact"><span className="mono">ПРОИЗВОДСТВЕННАЯ БАЗА</span><h2>Отгрузка и<br /><em>доставка.</em></h2><div className="contact-list"><div><span>Адрес</span><p>618900, Пермский край,<br />г. Лысьва, ул. Революции, 67</p></div><div><span>Телефоны</span><p>+7 (34249) 5-46-48<br />5-46-49, 5-46-20</p></div></div><div className="contact-dark-note"><Truck size={18} /> Собственный автопарк и доставка до места — по информации текущего сайта.</div></div></section></main>; }
...
function NotFoundPage() { return <main className="not-found wrap"><span className="mono">404 / СТРАНИЦА НЕ НАЙДЕНА</span><h1>Такой страницы<br /><em>пока нет.</em></h1><Link className="button button-primary" href="/">Вернуться на главную <ArrowLeft size={17} /></Link></main>; }
